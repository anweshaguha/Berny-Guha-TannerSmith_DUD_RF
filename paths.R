testfun <- local_attributions <- function(x, ...)
...
# average_yhats <- calculate_1d_changes(explainer_rf, baked2[1,3:49], explainer_rf$data, predict_function = yhat)
# diffs_1d <- sapply(seq_along(average_yhats), function(i) {
#  sqrt(mean((average_yhats[[i]] - .568)^2))
# })
# 
# feature_path <- create_ordered_path(diffs_1d, order = NULL, names(average_yhats))